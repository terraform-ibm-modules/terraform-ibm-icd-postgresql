...
| <a name="input_existing_hpcs_instance_guid"></a> [existing\_hpcs\_instance\_guid](#input\_existing\_hpcs\_instance\_guid) | The GUID of the Hyper Protect instance in which the key specified in var.kms\_key\_crn is coming from. Required if var.skip\_iam\_authorization\_policy is false in order to create an IAM Access Policy to allow Hyper Protect to access the newly created Postgresql database instance. | `string` | `null` | no |
...
| <a name="input_region"></a> [region](#input\_region) | The region postgresql is to be created on. The region must support BYOK if kms\_key\_crn is used | `string` | `"us-south"` | no |
...
| <a name="input_skip_iam_authorization_policy"></a> [skip\_iam\_authorization\_policy](#input\_skip\_iam\_authorization\_policy) | Set to true to skip the creation of an IAM authorization policy that permits the PostgreSQL database instance created to read the encryption key from the HPCS instance in `existing_hpcs_instance_guid`. | `bool` | `false` | no |
