...
Key Management Services like Key Protect or Hyper Protect Crypto Service (HPCS) can be used for disk encryption.  [key\_protect\_key\_crn](#input\_key\_protect\_key\_crn) can only be specified during database creation.

Key Protect BYOK can be used for disk encryption of backups by specifying [backup\_encryption\_key\_crn](#input\_backup\_encryption\_key\_crn) at database creation. BYOK for backups is available only in us-south, us-east and eu-de regions. Only keys in the us-south and eu-de are durable to region failures. To ensure that your backups are available even if a region failure occurs, you must use a key from us-south or eu-de, regardless of your deployment's location.

For more information, refer to the [disk encryption](https://cloud.ibm.com/docs/cloud-databases?topic=cloud-databases-key-protect#using-the-key-protect-key) documentation.
