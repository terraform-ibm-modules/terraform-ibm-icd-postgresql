...
locals {
  ...
  postgresql_key_name      = var.prefix != null ? "${var.prefix}-${var.postgresql_key_name}" : var.postgresql_key_name
  postgresql_key_ring_name = var.prefix != null ? "${var.prefix}-${var.postgresql_key_ring_name}" : var.postgresql_key_ring_name

  kms_key_crn = var.existing_kms_key_crn != null ? var.existing_kms_key_crn : module.kms[0].keys[format("%s.%s", local.postgresql_key_ring_name, local.postgresql_key_name)].crn
}
...
module "kms" {
  ...
  keys = [
    {
      key_ring_name         = local.postgresql_key_ring_name
      existing_key_ring     = false
      force_delete_key_ring = true
      keys = [
        {
          key_name                 = local.postgresql_key_name
          standard_key             = false
